...
def load_wav(
    ...
    """
    # Prefer torchaudio, but fall back to soundfile on environments where
    # torchcodec/ffmpeg dynamic libs are unavailable (common on CPU-only setups).
    global _AUDIO_FALLBACK_WARN_COUNT

    try:
        wav, sr = torchaudio.load(wav_path)
    except Exception as exc:
        _AUDIO_FALLBACK_WARN_COUNT += 1
        if _AUDIO_FALLBACK_WARN_COUNT <= 2:
            logging.warning(
                "torchaudio.load failed for [%s], falling back to soundfile. "
                "Reason (showing first 2 only): %s",
                wav_path,
                exc,
            )
